...
class Student(Base):
    ...
    # Связь 1-к-1 с User через user_id
    user_id = Column(Integer, ForeignKey('users.id'), primary_key=True)
    group_id = Column(Integer, ForeignKey('groups.id'))  # Принадлежность к группе

    # Связи ORM
    user = relationship("User", back_populates="student_profile")
    group = relationship("Group", back_populates="students")
    submissions = relationship("AssignmentSubmission", back_populates="student")  # Сданные задания
...
class Group(Base):
    ...
    subjects = relationship("Subject", secondary=group_subject, back_populates="groups")  # Предметы группы
...
class Subject(Base):
    ...
    groups = relationship("Group", secondary=group_subject, back_populates="subjects")  # Группы, изучающие предмет
...
class AssignmentSubmission(Base):
    ...
    student_id = Column(Integer, ForeignKey('students.user_id'), nullable=False)  # Студент
